...
export function ChatPanel({
  ...
  return (
    <Card
      className={cn(
        "glass-panel flex h-full flex-col overflow-hidden border-white/55 bg-white/85 shadow-[0_24px_80px_rgba(15,23,42,0.12)]",
    ...
    >
      ...
      <CardHeader className="border-b border-slate-200/70 pb-4 sm:pb-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            {badgeLabel ? <Badge className="mb-0 rounded-full border-0 bg-[#1E3A8A]/10 px-3 py-1 text-[#1E3A8A]">{badgeLabel}</Badge> : null}
            {hasHeaderText ? (
              <>
                {title ? <CardTitle className="mt-3 text-xl font-semibold text-slate-900 sm:text-2xl">{title}</CardTitle> : null}
